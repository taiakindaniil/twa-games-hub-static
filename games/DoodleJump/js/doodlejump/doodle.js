...
(Doodle = Doodle || {}).CalibrateState = {
    ...
    create: function() {
        ...
        this.doneButton.events.onInputUp.add(function() {
            14 == this.doneButton.frame && (this.state.start("Settings"),
            this.doneButton.loadTexture("atlas2", "donebtn"))
        }, this),
        ...
        this.setButton.events.onInputUp.add(function() {
            16 == this.setButton.frame && (this.calibrateSet(),
            this.setButton.loadTexture("atlas2", "setbtn"))
        }, this),
        ...
        for (var t = 0; 5 > t; t++)
            platform = new Phaser.Sprite(this.game,0 + 140 * t,this.game.world.height - 180 - t % 2 * 30,"atlas","platform0"),
        ...
        function t() {
            if (e) {
                ...
                i.cursorContr || i.game.paused || !i.player.alive || (i.player.body.velocity.x = 40 * s.gamma * Math.cos(s.beta * Math.PI / 180) + i.calibratedValue,
                40 * s.gamma * Math.cos(s.beta * Math.PI / 180) > 0 ? i.player.frame < 2 && i.player.scale.setTo(-1, 1) : i.player.frame < 2 && i.player.scale.setTo(1, 1))
            }
    ...
    initIOScontrol: function() {
        ...
        if (t && e) {
            ...
            (0,
            window[i])("attachEvent" == i ? "onmessage" : "message", function(t) {
                var e = t[t.message ? "message" : "data"];
                console.log("game", e),
                e.mssg && "deviceorientation" == e.mssg && "Calibrate" == Doodle.game.state.current && (console.log(e.event.alpha, e.event.beta, e.event.gamma),
                s.cursorContr || s.game.paused || !s.player.alive || (s.player.body.velocity.x = e.event.gamma * s.GYRO_SENSE * Math.cos(e.event.beta * Math.PI / 180) + s.calibratedValue,
                s.player.body.velocity.x > 0 ? s.player.frame < 2 && s.player.scale.setTo(-1, 1) : s.player.frame < 2 && s.player.scale.setTo(1, 1)))
            })
        }
    }
},
(Doodle = Doodle || {}).GameState = {
    ...
    create: function() {
        ...
        this.gameOverStats.anchor.setTo(0, .5),
        ...
        this.playAgainButton.anchor.setTo(.5),
        ...
        spaceBar.onDown.add(function() {
            this.gameOverGroup.visible ? (this.state.start("Game"),
            this.playAgainButton.loadTexture("atlas", "playAgain_01"),
            this.writeScore()) : this.adTriggered = !0
        }, this),
        this.playAgainButton.events.onInputUp.add(function() {
            7 == this.playAgainButton.frame && (this.adTriggered ? (this.state.start("Game"),
            this.playAgainButton.loadTexture("atlas", "playAgain_01"),
            this.writeScore()) : (this.gameOver(),
            this.adTriggered = !0))
        }, this),
        ...
        this.menuButton.events.onInputUp.add(function() {
            5 == this.menuButton.frame && (this.adTriggered ? (this.state.start("Menu"),
            this.menuButton.loadTexture("atlas", "menu_01"),
            this.writeScore()) : (this.gameOver(),
            this.adTriggered = !0))
        }, this),
        ...
        this.loadScoreMarks();
        var t, e = (navigator.userAgent.indexOf("AppleWebKit"),
        /iPad|iPhone|iPod/.test(navigator.userAgent) && window.MSStream,
        ...
        function e() {
            if (t) {
                ...
                s.cursorContr || s.game.paused || !s.player.alive || (s.player.body.velocity.x = i.gamma * s.GYRO_SENSE * Math.cos(i.beta * Math.PI / 180) + s.calibratedValue,
                i.gamma * s.GYRO_SENSE * Math.cos(i.beta * Math.PI / 180) + s.calibratedValue > 0 ? s.player.frame < 2 && s.player.scale.setTo(-1, 1) : s.player.frame < 2 && s.player.scale.setTo(1, 1))
            }
        ...
        this.game.input.onUp.add(function() {
            this.game.paused && (this.resumeButton.getBounds().contains(this.game.input.x, this.game.input.y) && this.unPauseGame(),
    ...
    update: function() {
        this.player.y > 2400 && !this.adTriggered && (this.gameOver(),
        this.adTriggered = !0),
        this.player.alive && (this.cursors.left.isDown ? (this.cursorContr = !0,
        this.player.body.velocity.x >= 0 ? this.player.body.velocity.x = -this.ACCELERATION : (this.player.body.velocity.x -= this.ACCELERATION,
        this.player.body.velocity.x < -this.MAX_ACCELERATION && (this.player.body.velocity.x = -this.MAX_ACCELERATION)),
        this.player.frame < 2 && this.player.scale.setTo(1, 1)) : this.cursors.right.isDown ? (this.cursorContr = !0,
        this.player.body.velocity.x <= 0 ? this.player.body.velocity.x = this.ACCELERATION : (this.player.body.velocity.x += this.ACCELERATION,
        this.player.body.velocity.x > this.MAX_ACCELERATION && (this.player.body.velocity.x = this.MAX_ACCELERATION)),
        this.player.frame < 2 && this.player.scale.setTo(-1, 1)) : !this.cursors.right.isDown && !this.cursors.left.isDown & 0 != this.player.body.velocity.x && (Math.abs(this.player.body.velocity.x) > this.ACCELERATION ? this.player.body.velocity.x -= this.player.body.velocity.x / Math.abs(this.player.body.velocity.x) * this.ACCELERATION : this.player.body.velocity.x = 0)),
        this.player.body.velocity.y > 1080 && (this.player.body.velocity.y = 1080),
        this.player.y < .48 * this.game.world.height && this.player.alive && this.moveScreen(this.player.y - .48 * this.game.world.height),
        this.precedingPlatform.y + 2 * this.score > 0 && (this.score > 5e3 && !this.player.withBonus && Math.random() > .95 ? this.loadScene() : this.loadLevel()),
        this.player.alive && (this.platformPool.forEach(function(t) {
            t.top >= this.game.world.height - this.platformPool.y && t.kill()
        }, this),
        this.bonusPool.forEach(function(t) {
            t.top >= this.game.world.height - this.bonusPool.y && ("bonus2" == t.type && !t.used && t.alive && (this.stats.achievements[10][2] += 1,
            this.stats.achievements[10][2] >= 3 && this.achieveMentUnlock(10)),
            t.kill())
        }, this),
        this.obstaclePool.forEach(function(t) {
            t.top >= this.game.world.height - this.obstaclePool.y && (t.alive && 4 != t.type && (this.stats.achievements[4][2] += 1,
        ...
        this.inputNameListener && ("highscore" != document.activeElement.id && (this.player.playerTimer.remove(this.player.inputTimer),
        this.gameOverStats.text = "your name:" + this.name,
        this.inputNameListener = !1),
        document.getElementById("highscore").value != this.name && (document.getElementById("highscore").value = document.getElementById("highscore").value.replace(/\W/g, ""),
    ...
    gameOver: function(t, e) {
        if ("fearless" != this.name) {
            ...
            0 == t) {
                ...
                this.player.playerTimer.running && (this.stats.stats[4][1] += Math.round(this.player.playerTimer.seconds),
                ...
                this.game.device.android ? (this.appstore.visible = !1,
                this.googleplay.y = 725) : this.game.device.iOS && (this.googleplay.visible = !1,
                this.appstore.y = 725),
                this.game.world.setBounds(0, 0, 640, 2400),
                this.game.camera.follow(this.player, Phaser.Camera.FOLLOW_LOCKON, .2, .2),
            ...
            2 == t && (this.player.body.velocity.y < 0 && (this.player.body.velocity.y = 0),
            ...
            offsetY = 0,
            offsetTime = 0,
            6 == e.type && e.animations.currentAnim ? (offsetX = 0,
            offsetY = -80) : 4 == e.type && (offsetX = 0,
            offsetY = -25,
            offsetTime = 400),
            e.suckYou = !0,
            ...
            }, 1e3 - offsetTime, Phaser.Easing.Linear.None),
            ...
            0 == t && (this.player.body.velocity.y = -2080)
    },
    ...
    loadLevel: function() {
        for (; this.precedingPlatform.y + 2 * this.score > -100; ) {
            ...
            2 != t.type && (this.jetpackAppear < 0 && this.JETPACK_TIMES > 0 ? (this.JETPACK_TIMES--,
            t.hasBonusObject = 2,
            this.jetpackAppear = 1e4 * Math.random() + 1e4,
            this.spawnBonus(2, t)) : this.propellerAppear < 0 ? (t.hasBonusObject = 3,
            this.propellerAppear = 1e4 * Math.random() + 1e4,
            this.spawnBonus(3, t)) : Math.random() > .9 && (t.hasBonusObject = 0,
            this.spawnBonus(0, t))),
            this.precedingPlatform = t
        }
    },
    jumpState: function(t, e) {
        t.alive && e.alive && e.y + e.height / 2 + this.platformPool.y > t.bottom && t.body.velocity.y > 0 && (2 == e.type && (e.body.velocity.x = 0,
        e.body.allowGravity = !0,
        e.animations.add("breake", ["atlas", "platform2", "platformSheet_02", "platformSheet_03", "platformSheet_04"], 24, !1),
        e.play("breake"),
        ...
        2 != e.type && (0 != t.frame && 2 != t.frame || (t.frame += 1,
        this.player.playerTimer.add(20 * Phaser.Timer.SECOND / 60, function() {
            t.frame -= 1
        }, this)),
        3 == e.type ? this.sounds.white.play() : this.sounds.jump.play(),
        t.body.velocity.y = -1080,
        0 == e.type && (this.stats.achievements[12][2] += 1,
        this.stats.achievements[12][2] >= 200 && this.achieveMentUnlock(12)),
        1 == e.type && (this.stats.achievements[13][2] += 1,
    ...
    obstacleUSE: function(t, e) {
        t.alive && !t.withBonus && e.alive && (e.y + e.height / 2 + this.obstaclePool.y > t.bottom && t.body.velocity.y > 0 && 4 != e.type ? (e.body.allowGravity = !0,
        t.body.velocity.y = -1080,
        ...
        6 == e.type ? this.stats.stats[11][1] += 1 : this.stats.stats[9][1] += 1,
        this.sounds.jumponmonster.play()) : 4 == e.type || 6 == e.type ? (6 == e.type && (e.sounds.ufo_abduct.play(),
        this.stats.stats[25][1] += 1),
        4 == e.type && (e.sounds.black_hole.play(),
        this.stats.stats[26][1] += 1),
        this.gameOver(1, e)) : (this.gameOver(2, e),
        this.sounds.monster_hit.play(),
        this.stats.stats[24][1] += 1)),
        t.alive && t.withBonus && e.alive && 4 != e.type && 3 == t.bonusType && (e.kill(),
        this.achieveMentUnlock(16))
    },
    ...
    spawnBonus: function(t, e) {
        ...
        s ? s.reset(e.x, e.top + 5, "bonus" + t, e, this.score) : (s = new Doodle.Bonus(this.game,e.x,e.top + 5,"bonus" + t,e,this.score,this.sounds,this.stats),
        this.bonusPool.add(s))
    },
    spawnPlatform: function(t, e, s, i, a, o, h) {
        var n = this.platformPool.getFirstExists(!1);
        return n ? n.reset(e, s, i, a, o, h) : (n = new Doodle.Platform(t,e,s,i,a,o,h),
        this.platformPool.add(n)),
        n
    },
    spanwObstacle: function(t, e, s, i) {
        var a = this.obstaclePool.getFirstExists(!1);
        return a ? a.reset(e, s, i) : (a = new Doodle.Obstacle(t,e,s,i,this.sounds,this.stats,this.obstaclePool),
        this.obstaclePool.add(a)),
        a
    },
    loadScene: function() {
        var t = this.precedingPlatform.y - 2 * this.distanceBtwPlatforms(1);
        this.score >= 15e3 ? (index = this.hardScenesIndex,
        array = this.scenes0.hardScenes,
        this.hardScenesIndex++) : (index = this.easyScenesIndex,
        ...
        array[index].forEach(function(e, s) {
            if (e.type < 4) {
                ...
                this.spanwObstacle(this.game, 2 * e.x, t - 2 * e.y, e.type)
        }, this),
    ...
    playerShoot: function(t) {
        this.game.paused || (velx = 0,
        offsetX = 0,
        this.trunk.angle = 0,
        this.player.alive && !this.player.withBonus && (Math.random() > .5 ? this.sounds.shoot2.play() : this.sounds.shoot.play(),
        this.player.frame < 2 && (this.player.frame += 2,
        this.trunk.visible = !0),
        1 == t && (velx = 1e3 * (this.game.input.activePointer.position.x - this.game.world.width / 2) / (this.game.world.width / 2),
        ...
        this.player.shootTimer = this.player.playerTimer.add(20 * Phaser.Timer.SECOND / 60, function() {
            this.player.frame -= 2,
            this.player.body.velocity.x < 0 ? this.player.scale.setTo(1, 1) : this.player.scale.setTo(-1, 1),
            this.trunk.visible = !1,
            this.player.playerTimer.remove(this.player.shootTimer)
        }, this)))
    },
    spawnBullet: function(t, e, s, i, a) {
        ...
        o ? o.reset(e, s, i) : (o = new Doodle.Bullet(t,e,s,i,this.stats),
    ...
    enemyCheck: function(t, e) {
        return 6 == e.type && e.animations.currentAnim ? (hz = t.y + 40 < e.y + this.obstaclePool.y,
        hz && this.enemyKill(t, e),
        !1) : void 0
    },
    enemyKill: function(t, e) {
        4 != e.type && (e.suckYou && (tween.stop(),
        tween2.stop(),
        this.player.scale.setTo.x <= 0 ? this.player.scale.setTo(-1, 1) : this.player.scale.setTo(1, 1),
        this.player.alive = !0,
    ...
    pauseGame: function() {
        this.player.alive && (this.game.paused = !0,
        this.pausePanelGroup.y = 0)
    },
    ...
    setHighScore: function() {
        ...
        for (var t = 0; 11 > t; t++)
            if ("findIndex" == this.localTopScores[t][0]) {
                this.setScoreName = t,
                this.localTopScores[t][0] = this.name;
                break
            }
    ...
    loadScoreMarks: function() {
        this.localTopScores.forEach(function(t) {
            scoreMark = new Phaser.Sprite(this.game,640,960 - 2 * t[1] - .48 * this.game.world.height,"atlas","scoreMark"),
            this.scoreMarkPool.add(scoreMark),
            scoreMark.anchor.setTo(1),
            this.nameMark = this.add.bitmapText(630, 960 - 2 * t[1] - .48 * this.game.world.height - 5, "DoodleFont", t[0], 24),
    ...
    writeScore: function() {
        if (this.stats.stats[2][1] = Math.round(this.score),
    ...
    achieveMentUnlock: function(t, e) {
        1 != this.stats.achievements[t][3] && (e || this.achievementsOrder.push(t),
        null == this.achievementsOrder[1]) && (this.achievementsGroup = this.add.group(),
        this.achievementName = this.add.bitmapText(this.game.world.width / 2, 1015, "DoodleFont2", this.stats.achievements[t][0], 60),
        this.achievementName.anchor.setTo(.5),
        this.achievementHolder = this.add.sprite(this.game.world.width / 2, 1002, "atlas2", "achievementHolder"),
        this.achievementHolder.anchor.setTo(.5),
        ...
        tween12.onComplete.addOnce(function() {
            ...
            this.achievementsOrder.length > 0 && this.achieveMentUnlock(this.achievementsOrder[0], !0)
        }, this),
        tween1.start())
    },
    initIOScontrol: function() {
        ...
        if (t && e) {
            ...
            window[i])("attachEvent" == i ? "onmessage" : "message", function(t) {
                var e = t[t.message ? "message" : "data"];
                console.log("game", e),
                e.mssg && "deviceorientation" == e.mssg && "Game" == Doodle.game.state.current && (console.log(e.event.alpha, e.event.beta, e.event.gamma),
                s.cursorContr || s.game.paused || !s.player.alive || (s.player.body.velocity.x = e.event.gamma * s.GYRO_SENSE * Math.cos(e.event.beta * Math.PI / 180) + s.calibratedValue,
                s.player.body.velocity.x > 0 ? s.player.frame < 2 && s.player.scale.setTo(-1, 1) : s.player.frame < 2 && s.player.scale.setTo(1, 1)))
            })
        }
...
(Doodle = Doodle || {}).MenuState = {
    ...
    create: function() {
        ...
        for (var e = 0; e < t.length; e++) {
            ...
            case 1:
                s = "googleplay",
                i = "https://play.google.com/store/apps/details?id=com.lima.doodlejump&utm_source=web%20game&utm_medium=online&utm_campaign=html5_pilot"
            }
            var a = 85 + 140 * e
              , o = this["imageLink" + t[e]];
            (o = this.add.sprite(a, 900, "popupAtlas", s)).anchor.setTo(.5),
            o.inputEnabled = !0,
            o.link = i,
        ...
        this.playButton.events.onInputUp.add(function() {
            11 == this.playButton.frame && (this.camera.fade("#000000"),
        ...
        this.optionsButton = this.add.sprite(this.world.game.width - 140, this.world.game.height - 280, "atlas2", "opt"),
        ...
        this.scoresButton = this.add.sprite(this.world.game.width - 260, this.world.game.height - 280, "atlas2", "opt2"),
...
(Doodle = Doodle || {}).PreloadState = {
    preload: function() {
        var t = (this.game.add.image(0, 0, "loadingscreen"),
        ...
        this.startGame = !1,
        this.game.time.events.add(1.4 * Phaser.Timer.SECOND, function() {
            this.startGame && this.state.start("Menu"),
            this.startGame = !0
        }, this),
...
(Doodle = Doodle || {}).ScoresState = {
    ...
    create: function() {
        this.game.input.mouse.mouseWheelCallback = function(e) {
            t.backgroundScores.getBounds().contains(t.game.input.x, t.game.input.y) && t.scrollMenu.visible && t.scrollMenu.y <= 0 && t.scrollMenu.y >= t.scrollLimit && (t.scrollMenu.y += 40 * t.game.input.mouse.wheelDelta,
            ...
            t.backgroundScores2.getBounds().contains(t.game.input.x, t.game.input.y) && t.scrollMenu2.visible && t.scrollMenu2.y <= 0 && t.scrollMenu2.y >= t.scrollLimit2 && (t.scrollMenu2.y += 40 * t.game.input.mouse.wheelDelta,
        ...
        ;
        var t = this;
        if (this.localTopScores.length > 0 ? (this.localTopScores[0][0],
        this.localTopScores[0][1],
        this.compare = this.submittedValue != this.localTopScores[0][1]) : this.compare = !1,
        ...
        this.stats.achievements.forEach(function(t, e) {
            0 == t[3] ? icon = this.add.sprite(115, 285 + 105 * e, "atlas2", "ach") : icon = this.add.sprite(115, 285 + 105 * e, "atlas2", "ach" + e),
        ...
        this.menuButton.events.onInputUp.add(function() {
            5 == this.menuButton.frame && (this.state.start("Menu"),
            this.menuButton.loadTexture("atlas", "menu_01"))
        }, this),
        this.menuButton.events.onInputDown.add(function() {
            this.menuButton.loadTexture("atlas", "menu_02")
    ...
    update: function() {
        this.followScroll && this.input.y != this.startTouchY && this.scrollMenu.y <= 0 && this.scrollMenu.y >= this.scrollLimit && (this.scrollMenu.y += this.input.y - this.startTouchY,
        ...
        this.followScroll2 && this.input.y != this.startTouchY2 && this.scrollMenu2.y <= 0 && this.scrollMenu2.y >= this.scrollLimit2 && (this.scrollMenu2.y += this.input.y - this.startTouchY2,
    ...
    spawnText: function(t, e, s, i, a, o, h) {
        ...
        return n ? (n.reset(e, s),
        n.text = i,
        n.fontSize = a) : ((n = this.add.bitmapText(e, s, "DoodleFont", i, a)).text = i,
    ...
    setState: function(t) {
        ...
        "score" == t ? (this.backgroundScores.height = 810,
        this.scrollLimit = -318,
        ...
        this.scoresBtn.loadTexture("atlas2", "scoresBtn1")) : "stats" == t ? (this.statsBtn.loadTexture("atlas2", "statsBtn1"),
        ...
        }, this)) : "achievements" == t && (this.achievementsGroup.visible = !0,
    ...
    setLoad: function(t) {
        ...
        "local" == t && (this.localButton.loadTexture("atlas2", "localBtn1"),
        this.scrollMenu.visible = !0,
        this.setState("score")),
        "global" == t && (this.globalButton.loadTexture("atlas2", "globalBtn1"),
        this.scrollMenu2.visible = !0,
        this.dataTest && this.dataTest.forEach(function(t, e) {
...
(Doodle = Doodle || {}).SettingsState = {
    ...
    toggleShooting: function(t) {
        if (t) {
            if (this.directionalShootingValue = !0,
            "object" == typeof localStorage)
                try {
            ...
            if (this.directionalShootingValue = !1,
            "object" == typeof localStorage)
                try {
                    localStorage.setItem("DJ_directionalShooting", this.directionalShootingValue)
                } catch (t) {
...
(Doodle = Doodle || {}).Bonus = function(t, e, s, i, a, o, h, n) {
    ...
    this.bonusAction = function(t, e, s) {
        ...
        e.y + e.height / 2 + this.bonusPool.y > t.bottom && t.body.velocity.y > 0 && ("bonus0" != e.type || e.used || (e.used = !0,
        e.loadTexture("atlas", "bonus01"),
        0 != t.frame && 2 != t.frame || (t.frame += 1,
        t.playerTimer.add(20 * Phaser.Timer.SECOND / 60, function() {
            t.frame -= 1
        }, this)),
        h.spring.play(),
        ...
        "bonus3" != e.type || e.used || t.withBonus || (this.stats.stats[19][1] += 1,
        ...
        e.y = -10,
        e.used = !0,
        e.animations.add("prop", ["atlas", "propeller_03", "propeller_02", "propeller_04"], 24, !0),
        e.play("prop"),
        t.withBonus = !0,
        t.bonusType = 3,
        ...
        e.body.velocity.x = 0,
        this.playerTimer.add(80 * Phaser.Timer.SECOND / 60, function() {
            t.body.gravity.y = -1728
        }, this),
        this.playerTimer.add(170 * Phaser.Timer.SECOND / 60, function() {
            t.body.gravity.y = -1008
        }, this),
        this.playerTimer.add(195 * Phaser.Timer.SECOND / 60, function() {
            e.animations.stop(null, !0),
            e.loadTexture("atlas", "bonus3")
        }, this),
        this.playerTimer.add(200 * Phaser.Timer.SECOND / 60, function() {
            e.maxVel = 600,
            t.frame = 0,
            t.body.gravity.y = 0,
            t.bonusType = null,
        ...
        "bonus2" != e.type || e.used || t.withBonus || (this.stats.stats[18][1] += 1,
        ...
        e.y = 110,
        e.used = !0,
        e.animations.add("anim0", ["atlas", "bonus2anim_01", "bonus2anim_02", "bonus2anim_03"], 24, !0),
        e.animations.add("anim1", ["atlas", "bonus2anim_04", "bonus2anim_05", "bonus2anim_06"], 24, !0),
        e.animations.add("anim2", ["atlas", "bonus2anim_07", "bonus2anim_08", "bonus2anim_09"], 24, !0),
        e.play("anim0"),
        t.withBonus = !0,
        e.body.velocity.x = 0,
        t.body.velocity.y = -1080,
        t.body.gravity.y = -2952,
        this.playerTimer.add(30 * Phaser.Timer.SECOND / 60, function() {
            e.play("anim1")
        }, this),
        this.playerTimer.add(60 * Phaser.Timer.SECOND / 60, function() {
            t.body.gravity.y = -1728
        }, this),
        this.playerTimer.add(120 * Phaser.Timer.SECOND / 60, function() {
            t.body.gravity.y = -864
        }, this),
        this.playerTimer.add(170 * Phaser.Timer.SECOND / 60, function() {
            e.play("anim2")
        }, this),
        this.playerTimer.add(180 * Phaser.Timer.SECOND / 60, function() {
            e.maxVel = 600,
            e.animations.stop(null, !0),
...
Doodle.Bonus.prototype.reset = function(t, e, s, i, a) {
    ...
    this.used = !1,
    0 != i.body.velocity.x && (this.body.velocity.x = i.body.velocity.x,
    i.child = this);
...
Doodle.Bullet.prototype.kill = function() {
    Phaser.Sprite.prototype.kill.call(this),
    this._outOfBoundsFired && (this.stats.achievements[6][2] = 0,
    this.stats.achievements[7][2] = 0)
}
,
...
Doodle.Obstacle.prototype.reset = function(t, e, s) {
    ...
    11 == s && (this.health = 2),
    4 == s && (this.body.setSize(80, 80, this.width / 2 - 40, this.height / 2 - 40),
    this.game.world.sendToBack(this)),
    9 == s && (this.body.velocity.x = 360,
    this.scale.x *= -1),
    6 == s && (this.animations.currentAnim = null,
    Math.random() > .33 ? (this.animations.add("beam", ["atlas", "ufo_01", "ufo_02"], 24, !0),
    this.play("beam"),
    this.body.setSize(this.width, this.height, 0, 0)) : this.body.setSize(160, 70, this.width / 2 - 80, this.height / 2 - 35)),
    10 == s && (this.animations.add("fly", ["atlas", "obstacle10", "but1", "but2", "but3", "but4", "but3", "but2", "but1", "obstacle10"], 24, !0),
    this.play("fly"))
}
,
Doodle.Obstacle.prototype.update = function() {
    this.alive && 9 == this.type && (this.left >= this.game.world.width && this.body.velocity.x > 0 && (this.scale.x *= -1,
    this.body.velocity.x *= -1),
    this.left <= 0 && this.body.velocity.x < 0 && (this.scale.x *= -1,
    this.body.velocity.x *= -1),
    ...
    !this.alive || 7 != this.type && 8 != this.type && 10 != this.type && 11 != this.type || (this.newOffsetX = 20 * Math.sin(this.rangeX * Math.PI / 180),
    ...
    this.rangeY >= 360 && (this.rangeY -= 360)),
    this.alive && 6 == this.type && !this.suckYou && (this.newOffsetX = 40 * Math.sin(this.rangeX * Math.PI / 180),
...
Doodle.Obstacle.prototype.damage = function(t) {
    ...
    11 == this.type && (this.animations.add("bloody", ["atlas", "green0", "green1", "green2", "green3", "green2", "green1", "green0"], 24, !0),
    this.play("bloody")),
    this.alive || (6 == this.type ? (this.sounds.ufo_kill.play(),
    this.stats.stats[10][1] += 1) : (this.sounds.monster_kill.play(),
    this.stats.stats[8][1] += 1),
...
Doodle.Obstacle.prototype.kill = function() {
    ...
    this.pool.forEachAlive(function(s) {
        6 == s.type && (t += 1),
        7 != s.type && 8 != s.type && 9 != s.type && 10 != s.type && 11 != s.type || (e += 1)
    }, this),
...
Doodle.Platform.prototype.reset = function(t, e, s, i, a, o) {
    ...
    this.body.allowGravity = !1,
    this.body.immovable = !0,
    null == o && (2 != i.type && Math.random() < .2 && (this.type = 2,
    this.loadTexture("atlas", "platform2")),
    a > 5500 && 100 * Math.random() < a / 400 && (this.moving = !0,
    2 != this.type && (this.type = 1,
    this.loadTexture("atlas", "platform1")),
    this.setVelocity = 60 * (.5 + .5 * Math.random() + a / 3e4) * 2,
    this.setVelocity > 300 ? this.body.velocity.x = 300 : this.body.velocity.x = this.setVelocity)),
    null != o && (this.loadTexture("atlas", "platform" + o),
    this.type = o,
    1 == o && (this.setVelocity = 60 * (.5 + .5 * Math.random() + a / 3e4) * 2,
    this.setVelocity > 300 ? this.body.velocity.x = 300 : this.body.velocity.x = this.setVelocity))
}
,
Doodle.Platform.prototype.update = function() {
    this.alive && (this.right >= this.game.world.width && (this.body.velocity.x > 0 && (this.body.velocity.x *= -1),
    this.child && (this.child.body.velocity.x *= -1)),
    this.left <= 0 && (this.body.velocity.x < 0 && (this.body.velocity.x *= -1),
    this.child && (this.child.body.velocity.x *= -1)),
